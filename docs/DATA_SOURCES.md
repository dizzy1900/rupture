...
| **NGL GNSS** (Nevada Geodetic Laboratory daily tenv3) | HTTPS `https://geodesy.unr.edu/gps_timeseries/IGS14/tenv3/IGS14/<SITE>.tenv3` (the older documented path under `/gps_timeseries/tenv3/IGS14/` 404'd on 2026-09-13). No API key. | tenv3 text | no SPDX; NGL asks for citation of Blewitt, Hammond & Kreemer 2018, doi:10.1029/2018EO104623 | parent series MB-scale; committed fixture is a 21-line prefix | daily final (IGS orbits ~14 d lag); rapid ~1 d (documented, not measured here) | `data/fixtures/gnss/ngl/` cut of station P595; live fetch is not cached in git | `source=ngl-gnss`, source URL, `retrieved_at`, sha256 of the cut, parent sha256, licence/citation as published |
| **USGS real-time earthquake GeoJSON feeds** | `https://earthquake.usgs.gov/earthquakes/feed/v1.0/summary/all_day.geojson` and siblings (live). Offline tests parse a one-day cut of the committed ComCat Ridgecrest GeoJSON, not a live poll. | GeoJSON | US Government work, public domain (USGS) | tens of kB per day | minutes | `data/fixtures/usgs_feed/`; live fetch is not cached in git | `source=usgs-realtime-feed`, feed or parent URL, `retrieved_at`, sha256, licence |
...
| `gnss/ngl/P595.tenv3.head` | header plus first 21 data lines of NGL IGS14 daily tenv3 for PBO station P595 (Ridgecrest area); byte-exact prefix of the 2026-09-13 parent file | NGL GNSS |
| `usgs_feed/all_day.ridgecrest-2019-07-06.geojson` | one UTC day cut of the committed ComCat Ridgecrest GeoJSON (parent sha256 recorded), used to exercise the real-time feed parser offline without a live poll | USGS ComCat (same licence) |
