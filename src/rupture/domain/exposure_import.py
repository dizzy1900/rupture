...
It lives in the domain because a published contract is domain, not application: ``domain`` may not
import from ``rupture.risk`` (the hexagonal rule, enforced by import-linter), so a schema that
``domain/contracts.py`` must register has to live here. ``rupture.risk.exposure_schema``
re-exports it for the adapter that enforces it.
